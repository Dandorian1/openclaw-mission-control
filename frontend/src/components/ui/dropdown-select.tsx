    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            "inline-flex h-10 w-auto cursor-pointer items-center justify-between gap-2 rounded-md border border-slate-300 bg-[color:var(--surface)] px-3 py-2 text-sm font-medium text-strong shadow-sm transition-colors hover:bg-[color:var(--surface-muted)] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50",
        ...
        >
          ...
          >
            {SelectedIcon ? (
              <SelectedIcon
                className={cn("h-4 w-4 text-muted", selectedIconClassName)}
              />
            ) : null}
      ...
      <PopoverContent
        ...
        sideOffset={6}
        className={cn(
          "w-[var(--radix-popover-trigger-width)] min-w-[12rem] overflow-hidden rounded-md border border-[color:var(--border)] bg-[color:var(--surface)] p-0 text-strong shadow-lg",
          contentClassName,
        )}
      >
