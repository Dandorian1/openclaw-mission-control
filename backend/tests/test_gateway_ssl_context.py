...
def test_create_ssl_context_disables_verification_when_allow_insecure_tls_true(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """SSL context should disable certificate verification when allow_insecure_tls is True (non-prod)."""
    import app.services.openclaw.gateway_rpc as rpc_module
    from app.core.config import Settings

    monkeypatch.setattr(rpc_module, "settings", Settings.model_construct(environment="dev"))
...
def test_create_ssl_context_respects_localhost_with_insecure_flag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """SSL context for localhost should respect allow_insecure_tls flag (non-prod)."""
    import app.services.openclaw.gateway_rpc as rpc_module
    from app.core.config import Settings
...
def test_create_ssl_context_respects_ip_address_with_insecure_flag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """SSL context for IP addresses should respect allow_insecure_tls flag (non-prod)."""
    import app.services.openclaw.gateway_rpc as rpc_module
    from app.core.config import Settings
...
def test_create_ssl_context_production_check_case_insensitive(
    ...
    """Production guard should be case-insensitive and cover both 'prod' and 'production'."""
    import app.services.openclaw.gateway_rpc as rpc_module
    from app.core.config import Settings

    for env_value in ("Production", "PRODUCTION", "production", "prod", "PROD", "Prod"):
